...
export async function POST(request: Request) {
  ...
  if (!apiKey || !project || !hash) {
    console.error("[/api/tour] Ennoia 환경변수 누락:", {
      apiKey: !!apiKey,
      project: !!project,
      hash: !!hash,
    });
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          error: `Ennoia API 오류: ${res.status}`,
    ...
    console.log("[/api/tour] 수집 응답(200):", fullContent.slice(0, 200));
    ...
    try {
      ...
      for (let i = 0; i < fullContent.length; i++) {
        if (fullContent[i] === "{") {
          ...
          depth--;
          if (depth === 0 && start !== -1) {
            blocks.push(fullContent.slice(start, i + 1));
            start = -1;
          }
        }
      }
      ...
      for (let i = blocks.length - 1; i >= 0; i--) {
        try {
          ...
          if ("summary" in candidate && "cards" in candidate) {
            parsed = candidate;
            break;
          }
        } catch (_e) {
      ...
      result = {
        ...
        excludedPlaces: Array.isArray(parsed.excludedPlaces)
          ? parsed.excludedPlaces
          : [],
